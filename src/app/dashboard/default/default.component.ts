...
export class DefaultComponent implements OnInit {
 public Pr: any;
  ...
  constructor(private route: ActivatedRoute, private cook:CookieService, private router: Router) {
    // Chart 5 Options
    this.chartOptions = {
  ...
  ngOnInit(): any {
    ...
    this.Pr = this.route.snapshot.paramMap.get('CVE');
    let url = this.route['_routerState'].snapshot.url
    console.log(this.Pr)
    console.log(this.cook.getAll())
    let Confirma : boolean = this.cook.check('Login'+this.Pr)
    if (Confirma == false)
    {
